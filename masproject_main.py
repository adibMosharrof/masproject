...
class Program:
    def init(self, grid_sz, num_resources, max_len, c_grid, st_pt, out_file):
        ...
        solutions, sol_val = milp.start()
        indexed_paths = milp.get_idx_paths()
        #gp.print_paths(gp.feasible_paths)

        p_grid = gp.gen_print_grid()
        start = gp.gen_print_start()
        print_results(out_file, solutions, indexed_paths, p_grid, start, max_len, num_resources, sol_val)

def print_results(out_file, solutions, paths, print_grid, start_point, max_len, num_resources, sol_val):
    f = open(out_file, "w+")
    ...
    for i in range(len(paths)):
        ...
        for j in range(len(paths[i])):
            f.write("Defender Resource #%d:\n" %(j))
            p_string = ""
            for k in range(len(paths[i][j])):
                p_string += str(paths[i][j][k]) + ", "
            f.write(p_string + "\n")
...
if __name__ == '__main__':
    ...
    program = Program()
    program.init(grid_size, num_defender, max_len, c_grid, start_pt, out_file)
